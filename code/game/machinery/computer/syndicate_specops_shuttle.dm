...
//		return
	...
	if (temp)
		...
		\nLocation: [syndicate_elite_shuttle_moving_to_station || syndicate_elite_shuttle_moving_to_mothership ? "Departing for [global.current_map.station_name] in ([syndicate_elite_shuttle_timeleft] seconds.)":syndicate_elite_shuttle_at_station ? "Station":"Dock"]<BR>
...
/obj/machinery/computer/syndicate_elite_shuttle/Topic(href, href_list)
	...
	if (href_list["sendtodock"])
		...
		usr << "\blue The Syndicate Elite shuttle will arrive on [global.current_map.station_name] in [(SYNDICATE_ELITE_MOVETIME/10)] seconds."
