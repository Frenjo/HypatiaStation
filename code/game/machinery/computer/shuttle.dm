/obj/machinery/computer/shuttle
	...
	attackby(var/obj/item/weapon/card/W as obj, var/mob/user as mob)
		...
		if (istype(W, /obj/item/weapon/card/id)||istype(W, /obj/item/device/pda))
			...
			if(emergency_shuttle.location() != 1 && user.get_active_hand() != W) // Updated to reflect 'shuttles' port. -Frenjo
				return 0
			switch(choice)
				if("Authorize")
					...
					if (src.auth_need - src.authorized.len > 0)
						...
						emergency_shuttle.online() // Updated to reflect 'shuttles' port. -Frenjo
			...
			if(!emagged && emergency_shuttle.location() == 1 && user.get_active_hand() == W) // Updated to reflect 'shuttles' port. -Frenjo
				switch(choice)
					if("Launch")
						world << "\blue <B>Alert: Shuttle launch time shortened to 10 seconds!</B>"
						//emergency_shuttle.settimeleft( 10 )
