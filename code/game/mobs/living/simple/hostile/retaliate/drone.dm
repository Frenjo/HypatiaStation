...
/mob/living/simple/hostile/retaliate/malf_drone/Life()
	...
	if(prob(1))
		visible_message(SPAN_WARNING("[icon2html(src, viewers(src))] [src] shudders and shakes as some of it's damaged systems come back online."))
	...
	if(prob(disabled ? 0 : 1))
		if(hostile_drone)
			visible_message(SPAN_INFO("[icon2html(src, viewers(src))] [src] retracts several targetting vanes, and dulls it's running lights."))
			hostile_drone = 0
		else
			visible_message(SPAN_WARNING("[icon2html(src, viewers(src))] [src] suddenly lights up, and additional targetting vanes slide into place."))
			hostile_drone = 1
	...
	if(health / maxHealth > 0.9)
		...
		if(!disabled)
			if(prob(50))
				...
				visible_message(SPAN_INFO("[icon2html(src, viewers(src))] [src] suddenly lies still and quiet."))
			disabled = rand(150, 600)
			walk(src, 0)

	if(exploding && prob(20))
		if(prob(50))
			visible_message(SPAN_WARNING("[icon2html(src, viewers(src))] [src] begins to spark and shake violenty!"))
		else
			visible_message(SPAN_WARNING("[icon2html(src, viewers(src))] [src] sparks and shakes like it's about to explode!"))
		make_sparks(3, TRUE, src)
