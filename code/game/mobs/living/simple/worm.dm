...
/mob/living/simple/space_worm/head/New(location, segments = 6)
	..()
...
/mob/living/simple/space_worm/proc/update_icon() //only for the sake of consistency with the other update icon procs
	if(stat == CONSCIOUS || stat == UNCONSCIOUS)
...
/mob/living/simple/space_worm/proc/AttemptToEat(atom/target)
...
/mob/living/simple/space_worm/proc/Attach(mob/living/simple/space_worm/attachement)
	if(!attachement)
		return
