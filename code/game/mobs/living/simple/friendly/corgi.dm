...
/mob/living/simple/corgi/show_inv(mob/user)
	user.set_machine(src)
	if(user.stat) return

	var/dat = 	"<div align='center'><b>Inventory of [name]</b></div><p>"
	if(inventory_head)
		dat +=	"<br><b>Head:</b> [inventory_head] (<a href='byond://?src=\ref[src];remove_inv=head'>Remove</a>)"
	else
		dat +=	"<br><b>Head:</b> <a href='byond://?src=\ref[src];add_inv=head'>Nothing</a>"
	if(inventory_back)
...
/mob/living/simple/corgi/attackby(var/obj/item/O, var/mob/user)
...
/mob/living/simple/corgi/Topic(href, href_list)
	if(usr.stat) return
...
/mob/living/simple/corgi/Ian/Life()
	..()
	...
	if(!stat && !resting && !buckled)
		...
		if(turns_since_scan > 5)
			...
			if(!movement_target || !(movement_target.loc in oview(src, 3)))
				movement_target = null
				stop_automated_movement = FALSE
				for(var/obj/item/reagent_holder/food/snacks/S in oview(src, 3))
...
/mob/living/simple/corgi/regenerate_icons()
	overlays = list()
	...
	if(!stat && !resting && !buckled)
		...
		if(turns_since_scan > 15)
			...
			for(var/mob/M in oviewers(7, src))
				if(istype(M, /mob/living/simple/corgi/Ian))
					if(M.client)
						alone = 0
						break
					else
			...
			if(alone && ian && puppies < 4)
				if(near_camera(src) || near_camera(ian))
					return
				new /mob/living/simple/corgi/puppy(loc)
