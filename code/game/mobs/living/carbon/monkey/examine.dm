/mob/living/carbon/monkey/get_examine_text()
	...
	if(isnotnull(wear_mask))
		. += SPAN_INFO("It has [html_icon(wear_mask)] <em>\a [wear_mask]</em> on its head.")
	if(isnotnull(back))
		. += SPAN_INFO("It has [html_icon(back)] <em>\a [back]</em> on its back.")
	if(isnotnull(l_hand))
		. += SPAN_INFO("It has [html_icon(l_hand)] <em>\a [l_hand]</em> in its left hand.")
	if(isnotnull(r_hand))
		. += SPAN_INFO("It has [html_icon(r_hand)] <em>\a [r_hand]</em> in its right hand.")
	if(isnotnull(handcuffed))
		. += SPAN_WARNING("It is [html_icon(handcuffed)] handcuffed!")
