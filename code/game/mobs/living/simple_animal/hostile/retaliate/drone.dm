...
/mob/living/simple_animal/hostile/retaliate/malf_drone/Destroy()
	...
	if(has_loot)
		...
		if(spawnees & 1)
			...
			C.origin_tech = list(RESEARCH_TECH_PROGRAMMING = rand(3, 6))

		if(spawnees & 2)
			C = new(src.loc)
			C.name = "Drone neural interface"
			C.origin_tech = list(RESEARCH_TECH_BIOTECH = rand(3, 6))

		if(spawnees & 4)
			C = new(src.loc)
			C.name = "Drone suspension processor"
			C.origin_tech = list(RESEARCH_TECH_MAGNETS = rand(3, 6))

		if(spawnees & 8)
			C = new(src.loc)
			C.name = "Drone shielding controller"
			C.origin_tech = list(RESEARCH_TECH_BLUESPACE = rand(3, 6))

		if(spawnees & 16)
			C = new(src.loc)
			C.name = "Drone power capacitor"
			C.origin_tech = list(RESEARCH_TECH_POWERSTORAGE = rand(3, 6))

		if(spawnees & 32)
			C = new(src.loc)
			C.name = "Drone hull reinforcer"
			C.origin_tech = list(RESEARCH_TECH_MATERIALS = rand(3, 6))

		if(spawnees & 64)
			C = new(src.loc)
			C.name = "Drone auto-repair system"
			C.origin_tech = list(RESEARCH_TECH_ENGINEERING = rand(3, 6))

		if(spawnees & 128)
			C = new(src.loc)
			C.name = "Drone plasma overcharge counter"
			C.origin_tech = list(RESEARCH_TECH_PLASMATECH = rand(3, 6))

		if(spawnees & 256)
			C = new(src.loc)
			C.name = "Drone targetting circuitboard"
			C.origin_tech = list(RESEARCH_TECH_COMBAT = rand(3, 6))

		if(spawnees & 512)
			C = new(src.loc)
			C.name = "Corrupted drone morality core"
			C.origin_tech = list(RESEARCH_TECH_SYNDICATE = rand(3, 6))
