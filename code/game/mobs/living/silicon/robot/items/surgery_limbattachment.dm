/obj/item/robot_part/attack(mob/living/carbon/human/M, mob/living/carbon/user)
	...
	if((user.zone_sel.selecting == "l_arm") && (istype(src, /obj/item/robot_part/l_arm)))
		...
		to_chat(user, SPAN_WARNING("That doesn't fit there!"))
		return ..()
	...
	if(S.status & ORGAN_DESTROYED)
		if(!(S.status & ORGAN_ATTACHABLE))
			to_chat(user, SPAN_WARNING("The wound is not ready for a replacement!"))
			return 0
		if(M != user)
			M.visible_message( \
		...
		if(do_mob(user, H, 100))
			...
			if(H == user && prob(25))
				to_chat(user, SPAN_WARNING("You mess up!"))
				S.take_damage(15)
