/mob/living/silicon/handle_topic(mob/user, datum/topic_input/topic, topic_result)
	...
	else if(topic.has("lawi")) // Toggling whether or not a law gets stated by the State Laws verb --NeoFite
	...
	else if(topic.has("laws")) // With how my law selection code works, I changed statelaws from a verb to a proc, and call it through my law selection panel. --NeoFite
		state_laws()