...
/mob/living/hitby(atom/movable/AM, speed = 5) //Standardization and logging -Sieve
	if(isobj(AM))
		...
		if(assailant && assailant.mob && ismob(assailant.mob))
			...
			attack_log += "\[[time_stamp()]\] <font color='orange'>Has been hit with a thrown [O], last touched by [M.name] ([assailant.ckey])</font>"
			M.attack_log += "\[[time_stamp()]\] <font color='red'>Hit [src.name] ([src.ckey]) with a thrown [O]</font>"
			if(!istype(src, /mob/living/simple/mouse))
				msg_admin_attack("[src.name] ([src.ckey]) was hit by a thrown [O], last touched by [M.name] ([assailant.ckey]) (<A href='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[src.x];Y=[src.y];Z=[src.z]'>JMP</a>)")
