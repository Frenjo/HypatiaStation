...
/datum/process/vote/do_work()
	if(isnotnull(mode))
		...
		if(mode == "gamemode" && global.PCticker.current_state >= 2)
	...
	if(!CONFIG_GET(/decl/configuration_entry/vote_no_default) && length(choices))
		...
		if(non_voters > 0)
			if(mode == "restart")
				...
				if(global.PCticker.master_mode in choices)
					choices[global.PCticker.master_mode] += non_voters
					if(choices[global.PCticker.master_mode] >= greatest_votes)
						greatest_votes = choices[global.PCticker.master_mode]
...
/datum/process/vote/proc/announce_result()
	...
	if(length(winners))
		...
		if((mode == "gamemode" && . == "extended") || !global.PCticker.hide_mode) // Announce Extended gamemode, but not other gamemodes
...
/datum/process/vote/proc/result()
		...
		switch(mode)
			if("restart")
				if(. == "Restart Round")
					restart = 1
			if("gamemode")
				if(global.PCticker.master_mode != .)
					world.save_mode(.)
					if(isnotnull(global.PCticker?.mode))
						restart = 1
					else
						global.PCticker.master_mode = .
			if("crew_transfer")
	...
	if(mode == "gamemode") //fire this even if the vote fails.
		if(!global.PCticker.roundstart_progressing)
			global.PCticker.roundstart_progressing = TRUE
...
/datum/process/vote/proc/initiate_vote(vote_type, initiator_key)
	if(isnull(mode))
		...
		switch(vote_type)
			...
			if("gamemode")
				if(global.PCticker.current_state >= GAME_STATE_SETTING_UP)
					return 0
				choices.Add(global.CTconfiguration.votable_modes)
			if("crew_transfer")
		...
		switch(vote_type)
			if("crew_transfer")
				SOUND_TO(world, 'sound/misc/alarm4.ogg')
			if("gamemode")
				SOUND_TO(world, 'sound/misc/alarm4.ogg')
			if("custom")
				SOUND_TO(world, 'sound/misc/alarm4.ogg')
...
/datum/process/vote/handle_topic(mob/user, datum/topic_input/topic)
	...
	switch(topic.get("vote"))
		if("close")
			...
			return
		if("cancel")
			if(isnotnull(user.client.holder))
				reset()
		if("toggle_restart")
			if(isnotnull(user.client.holder))
		...
		if("restart")
			if(CONFIG_GET(/decl/configuration_entry/allow_vote_restart) || isnotnull(user.client.holder))
				initiate_vote("restart", user.key)
		if("gamemode")
			if(CONFIG_GET(/decl/configuration_entry/allow_vote_mode) || isnotnull(user.client.holder))
				initiate_vote("gamemode", user.key)
		if("crew_transfer")
			if(CONFIG_GET(/decl/configuration_entry/allow_vote_restart) || isnotnull(user.client.holder))
				initiate_vote("crew_transfer", user.key)
		if("custom")
			if(isnotnull(user.client.holder))
				initiate_vote("custom", user.key)
		else
			submit_vote(user.ckey, round(topic.get_num("vote")))
	user.vote()
