...
/datum/process/air/do_work()
	...
	if(!global.PCair.process_internal()) //Runtimed.
		global.PCair.failed_ticks++

		if(global.PCair.failed_ticks > 5)
			to_world(SPAN_DANGER("RUNTIMES IN ATMOS TICKER. Killing air simulation!"))
			world.log << "### ZAS SHUTDOWN"

			message_admins("ZASALERT: Shutting down! status: [global.PCair.tick_progress]")
			log_admin("ZASALERT: Shutting down! status: [global.PCair.tick_progress]")

			processing_killed = TRUE
			global.PCair.failed_ticks = 0
...
/datum/process/air/proc/connect(turf/simulated/A, turf/simulated/B)
	...
	var/block = global.PCair.air_blocked(A, B)
