	...
	objective
		...
		capture
			...
			check_completion()
				if(target && target.current)
					if(target.current.stat == 2)
						if(CONFIG_GET(/decl/configuration_entry/require_heads_alive)) return 0
					else
						if(!target.current.handcuffed)
							return 0
				else if(CONFIG_GET(/decl/configuration_entry/require_heads_alive)) return 0
				return 1
		...
		steal
			...
			*/
			...
			/*burger
				steal_target = /obj/item/reagent_holder/food/snacks/human/burger
			...
			diamond_drill
				steal_target = /obj/item/pickaxe/drill/diamond
