	...
	objective
		...
		frame
			...
			check_completion()
				...
				if(!location)
					return 0
				if(!target.current:handcuffed && !istype(location, /turf/simulated/shuttle/floor/brig))
					return 0
		...
		capture
			...
			check_completion()
				if(target && target.current)
					if(target.current.stat == 2)
						if(CONFIG_GET(require_heads_alive)) return 0
					else
						if(!target.current.handcuffed)
							return 0
				else if(CONFIG_GET(require_heads_alive)) return 0
				return 1
		...
		escape
			...
			check_completion()
				...
				if(owner.current:handcuffed || istype(location, /turf/simulated/shuttle/floor/brig))
					return 0
		...
		steal
			...
			captainslaser
				steal_target = /obj/item/gun/energy/laser/captain
			...
			proc/gen_amount_goal(var/lowbound = 4, var/highbound = 6)
				...
				if (ticker)
					...
					if(ticker.current_state == GAME_STATE_SETTING_UP)
						...
						for(var/mob/living/carbon/human/P in GLOBL.mob_list)
							if(isnotnull(P.client) && !(P.mind in ticker.mode.changelings) && P.mind != owner)
								n_p ++
