...
/datum/disease/robotic_transformation/stage_act()
	...
	switch(stage)
		if(2)
			...
			if(prob(9))
				to_chat(affected_mob, SPAN_WARNING("Beep... boop.."))
			if(prob(9))
				to_chat(affected_mob, SPAN_WARNING("Bop... beeep..."))
		if(3)
			if(prob(8))
				to_chat(affected_mob, SPAN_WARNING("Your joints feel very stiff."))
				affected_mob.take_organ_damage(1)
			if(prob(8))
		...
		if(4)
			if(prob(10))
				to_chat(affected_mob, SPAN_WARNING("Your skin feels very loose."))
		...
		if(5)
			to_chat(affected_mob, SPAN_WARNING("Your skin feels as if it's about to burst off..."))
			...
			if(prob(40)) //So everyone can feel like robot Seth Brundle
				...
				if(istype(H) && !jobban_isbanned(affected_mob, "Robot"))
					H.Robotize()
				else
