...
/datum/wires/mulebot/UpdatePulsed(index)
	switch(index)
		if(WIRE_POWER1, WIRE_POWER2)
			holder.visible_message(SPAN_NOTICE("[html_icon(holder)] The charge light flickers."))
		if(WIRE_AVOIDANCE)
			holder.visible_message(SPAN_NOTICE("[html_icon(holder)] The external warning lights flash briefly."))
		if(WIRE_LOADCHECK)
			holder.visible_message(SPAN_NOTICE("[html_icon(holder)] The load platform clunks."))
		if(WIRE_MOTOR1, WIRE_MOTOR2)
			holder.visible_message(SPAN_NOTICE("[html_icon(holder)] The drive motor whines briefly."))
		else
			holder.visible_message(SPAN_NOTICE("[html_icon(holder)] You hear a radio crackle."))
