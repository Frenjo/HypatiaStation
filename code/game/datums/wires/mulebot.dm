/datum/wires/mulebot
	random = 1
	holder_type = /mob/living/bot/mulebot
	wire_count = 10
...
/datum/wires/mulebot/CanUse(mob/living/L)
	var/mob/living/bot/mulebot/M = holder
	if(M.open)
		return 1
	return 0
...
/datum/wires/mulebot/UpdatePulsed(index)
	switch(index)
		if(WIRE_POWER1, WIRE_POWER2)
			holder.visible_message(SPAN_NOTICE("[icon2html(holder, viewers(holder))] The charge light flickers."))
		if(WIRE_AVOIDANCE)
			holder.visible_message(SPAN_NOTICE("[icon2html(holder, viewers(holder))] The external warning lights flash briefly."))
		if(WIRE_LOADCHECK)
			holder.visible_message(SPAN_NOTICE("[icon2html(holder, viewers(holder))] The load platform clunks."))
		if(WIRE_MOTOR1, WIRE_MOTOR2)
			holder.visible_message(SPAN_NOTICE("[icon2html(holder, viewers(holder))] The drive motor whines briefly."))
		else
			holder.visible_message(SPAN_NOTICE("[icon2html(holder, viewers(holder))] You hear a radio crackle."))
