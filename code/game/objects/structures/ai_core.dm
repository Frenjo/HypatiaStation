					...
					if("AICARD")
						...
						if(!length(target_fixer.contents))
							if(!length(card.contents))
								...
								if(A.stat == DEAD)
									target_fixer.add_overlay("ai-fixer-404")
								else
									target_fixer.add_overlay("ai-fixer-full")
								target_fixer.remove_overlay("ai-fixer-empty")
								A.cancel_camera()
							...
							if(!length(card.contents) && target_fixer.occupant && !target_fixer.active)
								...
								target_fixer.add_overlay("ai-fixer-empty")
								if(target_fixer.occupant.stat == DEAD)
									card.icon_state = "aicard-404"
									target_fixer.remove_overlay("ai-fixer-404")
								else
									card.icon_state = "aicard-full"
									target_fixer.remove_overlay("ai-fixer-full")
