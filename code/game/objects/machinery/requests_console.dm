							...
							if("2")		//High priority
								...
								if(!Console.silent)
									...
									for(var/mob/O in hearers(5, Console.loc))
										O.show_message("[html_icon(Console)] *The Requests Console beeps: 'PRIORITY Alert in [department]'")
		...
		//						Console.messages += "<B><FONT color='red'>Extreme Priority message from [ckey(department)]</FONT></B><BR>[message]"
								...
								if(!Console.silent)
									...
									for(var/mob/O in hearers(4, Console.loc))
										O.show_message("[html_icon(Console)] *The Requests Console beeps: 'Message from [department]'")
				...
				for(var/mob/O in hearers(4, src.loc))
					O.show_message("[html_icon(src)] *The Requests Console beeps: 'NOTICE: No server detected!'")
