...
"}
				...
				if(4.0)
					...
					if(istype(src.active2, /datum/data/record) && GLOBL.data_core.medical.Find(src.active2))
	...
	if((usr.contents.Find(src) || (in_range(src, usr) && isturf(src.loc))) || (issilicon(usr)))
		...
		if(src.authenticated)
			...
			if(href_list["add_c"])
				...
				while(src.active2.fields[text("com_[]", counter)])
					counter++
				src.active2.fields[text("com_[counter]")] = text("Made by [authenticated] ([rank]) on [time2text(world.realtime, "DDD MMM DD hh:mm:ss")], [GLOBL.game_year]<BR>[t1]")
