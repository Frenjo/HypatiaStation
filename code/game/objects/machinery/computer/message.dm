...
/obj/machinery/computer/message_monitor/attack_hand(mob/living/user)
	...
	switch(screen)
		...
		if(0)
			...
			if((isAI(user) || isrobot(user)) && (!isemptylist(user.mind.special_roles) && user.mind.original == user))
...
/obj/machinery/computer/message_monitor/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && isturf(src.loc))) || (issilicon(usr)))
		...
		if (href_list["hack"])
			if((isAI(usr) || isrobot(usr)) && (!isemptylist(usr.mind.special_roles) && usr.mind.original == usr))
				src.hacking = 1
				src.screen = 2
						...
						if(isnull(PDARec))
							...
							if(!customrecepient.silent)
								...
								for (var/mob/O in hearers(3, customrecepient.loc))
									O.show_message("[icon2html(customrecepient, O)] *[customrecepient.ttone]*")
