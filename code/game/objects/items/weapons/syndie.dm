/obj/item/syndie
	icon = 'icons/obj/weapons/syndieweapons.dmi'
...
/obj/item/syndie/c4explosive/proc/detonate()
	...
	spawn(50)
		...
		for(var/dirn in cardinal)		//This is to guarantee that C4 at least breaks down all immediately adjacent walls and doors.
			var/turf/closed/wall/T = get_step(src,dirn)
...
/obj/item/syndie/c4detonator/attack_self(mob/user)
	switch(src.icon_state)
		if("c-4detonator_0")
			...
			to_chat(user, SPAN_INFO("You flick open the lighter."))
			...
			if(!pr_open)
				...
				switch(alert(user, "What would you like to do?", "Lighter", "Press the button.", "Close the lighter."))
					...
					if("Close the lighter.")
						...
						to_chat(user, SPAN_INFO("You close the lighter."))
				pr_open = 0