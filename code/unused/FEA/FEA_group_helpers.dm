/turf/open/proc/find_group()
	...
	var/turf/open/floor/north = get_step(src,NORTH)
	var/turf/open/floor/south = get_step(src,SOUTH)
	var/turf/open/floor/east = get_step(src,EAST)
	var/turf/open/floor/west = get_step(src,WEST)
...
//	to_world("[north_votes], [south_votes], [east_votes]")
