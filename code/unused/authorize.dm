...
/client/proc/authorize()
	...
	if (!CONFIG_GET(/decl/configuration_entry/enable_authentication))
		src.authenticated = 1
		return
...
/client/proc/goonauth()
	...
	if (CONFIG_GET(/decl/configuration_entry/enable_authentication))	//so that this verb isn't used when its goon only
