	...
	folder
		...
		proc
			...
			remove_file(datum/computer/R)
				if(holder && !holder.read_only || !R)
					//to_world("Removing file [R]. File_used: [src.holder.file_used]")
					src.contents.Remove(R)
					src.holder.file_used -= src.size
					src.size -= R.size
					src.holder.file_used += src.size
					src.holder.file_used = max(src.holder.file_used, 0)
					//to_world("Removed file [R]. File_used: [src.holder.file_used]")
					return 1
				return 0
	file
		name = "File"
	...
	proc
		return_text()
			...
			if(!(holder in src.master.contents))
				//to_world("Holder [holder] not in [master] of prg:[src]")
		...
		allowed(mob/M)
			...
			if(issilicon(M))
				...
				if(src.check_access(H.equipped()) || src.check_access(H.id_store))
					return 1
			else if(ismonkey(M))
