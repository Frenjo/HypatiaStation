...
/obj/effect/trap/aoe/rocksfall/proc/pick_rock_types() //since we may want subtypes of the trap with completely different rock types, which is best done this way
	...
	varieties = typesof(/obj/item/ore)
	varieties -= /obj/item/ore/diamond  //don't want easily available rare ores, hmm?
	varieties -= /obj/item/ore/uranium
	varieties -= /obj/item/ore/slag     //that'd be just stupid
...
/obj/effect/trap/aoe/rocksfall/activate()
	...
	if(target_type == "turf")
		for(var/i=0,i<rocks_amt,i++)
			...
			var/obj/item/ore/rock = new rock_type(hit_loc)
	...
	if(target_type == "mob")
		for(var/i=0,i<rocks_amt,i++)
			...
			var/rock_type = pick(rocks_type)
			var/obj/item/ore/rock = new rock_type(hit_loc.loc)
			if(prob(rocks_hit_chance))
