...
/datum/tension
	...
	proc/process()
		...
		if(CONFIG_GET(/decl/configuration_entry/Tensioner_Active))
					...
					if(prob(1) || forcenexttick)
						...
						if(prob(10) || forcenexttick)
							...
							for (var/client/C in admin_list)
								C << "<font color='red' size='3'><b> The tensioner wishes to create additional antagonists!  Press (<a href='byond://?src=\ref[tension_master];Supress=1'>this</a>) in 60 seconds to abort!</b></font>"
	...
	Topic(href, href_list)
		...
		if(!usr.client.holder)
			message_admins("\red [key_name(usr)] tried to use the tensioner without authorisation.")
			log_admin("[key_name(usr)] tried to use the tensioner without authorisation.")
			return
		...
		if (href_list["makeTratior"])
			...
			CONFIG_SET(Tensioner_Active, !CONFIG_GET(/decl/configuration_entry/Tensioner_Active))
...
//			for(var/i = 0, i < 4, i++) // Four rune words
	...
	/*
		if (emergency_shuttle.direction == 1 && emergency_shuttle.online == 1)
			...
			to_world("\blue <B>Alert: The shuttle is going back!</B>")
