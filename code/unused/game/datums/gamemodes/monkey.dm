...
/datum/game_mode/monkey/announce()
	to_world("<B>The current game mode is - Monkey!</B>")
	to_world("<B>Some of your crew members have been infected by a mutageous virus!</B>")
	to_world("<B>Escape on the shuttle but the humans have precedence!</B>")
...
/datum/game_mode/monkey/declare_completion()
	...
	if (monkeywin)
		...
		to_world("<FONT size=3 color=red><B>The monkeys have won! Humanity is doomed!</B></FONT>")
		...
		to_world("<FONT size=3 color=red><B>The Research Staff has stopped the monkey invasion!</B></FONT>")
	..()
	return 1
	...
	for(var/mob/living/carbon/monkey/monkey_player in mob_list)
		for(var/datum/disease/D in monkey_player.viruses)
			if (istype(D, /datum/disease/jungle_fever) && monkey_player.ckey)
				to_world("<B>[monkey_player.ckey] was played infested [monkey_player]. [monkey_player.stat == 2 ? "(DEAD)" : ""]</B>")
	return 1
