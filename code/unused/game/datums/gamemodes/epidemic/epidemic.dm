...
/datum/game_mode/epidemic/announce()
	to_world("<B>The current game mode is - Epidemic!</B>")
	to_world("<B>A deadly epidemic is spreading on the station. Find a cure as fast as possible, and keep your distance to anyone who speaks in a hoarse voice!</B>")
...
/datum/game_mode/epidemic/post_setup()
	...
	if(length(crew) < 2)
		to_world("\red There aren't enough players for this mode!")
		to_world("\red Rebooting world in 5 seconds.")
...
/datum/game_mode/epidemic/process()
	if(stage == 1 && cruiser_seconds() < 60 * 30)
		...
		priority_announce(
			"Inbound cruiser detected on collision course. Scans indicate the ship to be armed and ready to fire. Estimated time of arrival: 5 minutes.",
			"[station_name()] Early Warning System"
		)
		stage = 3
...
/datum/game_mode/epidemic/declare_completion()
	if(finished == 1)
		...
		to_world("\red <FONT size = 3><B> The virus outbreak was contained! The crew wins!</B></FONT>")
	else if(finished == 2)
		feedback_set_details("round_end_result","loss - rev heads killed")
		to_world("\red <FONT size = 3><B> The crew succumbed to the epidemic!</B></FONT>")
	..()
	return 1