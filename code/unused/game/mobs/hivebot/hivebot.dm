...
/mob/living/silicon/hivebot/attack_alien(mob/living/carbon/alien/humanoid/M as mob)
	...
	if (M.a_intent == "grab")
		...
		src.grabbed_by += G
		G.synch()
		playsound(src.loc, 'sound/weapons/melee/thudswoosh.ogg', 50, 1, -1)
		...
		*/
			...
			playsound(src.loc, 'sound/weapons/melee/slashmiss.ogg', 25, 1, -1)
			for(var/mob/O in viewers(src, null))
		...
		if(!(src.lying))
			...
			if (randn <= 40)
				...
				playsound(src.loc, 'sound/weapons/melee/slash.ogg', 50, 1, -1)
				for(var/mob/O in viewers(src, null))
