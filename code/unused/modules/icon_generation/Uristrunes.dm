...
/proc/create_border_image(icon/input, border_color = "#000000", fill_color = "#000000", border_alpha = 255, fill_alpha = 255)
	var/icon/I = icon('icons/effects/decals/uristrunes.dmi', "blank")
...
/proc/get_rune(rune_bits, animated = 0)
	...
	var/icon/base = icon('icons/effects/decals/uristrunes.dmi', "")

	for(var/i = 0, i < 10, i++)
		if(BITTEST(rune_bits, i))
			base.Blend(icon('icons/effects/decals/uristrunes.dmi', "rune-[1 << i]"), ICON_OVERLAY)
...
// Commented these out because debug procs and I've debugged thoroughly. -Frenjo
/*
...
/mob/verb/create_rune_custom(rune as num, color1 as color, border1 as color, color2 as color, border2 as color, alpha1 as num, alpha2 as num)
	...
	for(var/i = 0, i < 10, i++)
		if(rune & (1 << i))
			I.Blend(icon('icons/effects/decals/uristrunes.dmi', "rune-[1 << i]"), ICON_OVERLAY)
