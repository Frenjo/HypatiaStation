...
/datum/gas_mixture/proc/check_me_then_temperature_turf_share(turf/open/sharer, conduction_coefficient)
...
/datum/gas_mixture/proc/compare(datum/gas_mixture/sample)
	...
	if(total_moles() > MINIMUM_AIR_TO_SUSPEND)
		if((abs(temperature-sample.temperature) > MINIMUM_TEMPERATURE_DELTA_TO_SUSPEND) && \
			...
			//to_world("temp fail [temperature] & [sample.temperature]")
			return 0
	var/check_moles
	if(length(sample.trace_gases))
		for(var/datum/gas/trace_gas in sample.trace_gases)
