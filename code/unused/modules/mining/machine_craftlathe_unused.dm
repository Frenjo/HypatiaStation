...
/obj/machinery/autolathe2/attack_hand(mob/user as mob)
	...
	for(i = 1; i <= craft_contents.len; i++)
		...
		if (CI == selected)
			...
			dat += text("<A href='byond:://?src=\ref[src];select=[i]'>[CI.name]</a> ([CI.amount])<br>")
	...
	for (i = 0; i < 3; i++)
		...
		for (j = 1; j <= 3; j++)
			k = i * 3 + j
			if (current_recipe[k])
				dat += text("	<td><A href='byond:://?src=\ref[src];remove=[k]'>[current_recipe[k]]</a></td>")
			else
				dat += text("	<td><A href='byond:://?src=\ref[src];add=[k]'>----</a></td>")
		dat += text("	</tr>")
	dat += text("	</table>")
	...
	if (make)
		dat += text("<A href='byond:://?src=\ref[src];make=[1]'>[make.name]</a>")
	else
