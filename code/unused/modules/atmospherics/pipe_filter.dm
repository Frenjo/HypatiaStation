...
/obj/machinery/pipefilter/attackby(obj/item/W, mob/user as mob)
	...
	if(istype(W, /obj/item/card/emag) && (!emagged))
		...
		add_overlay(image('icons/obj/pipes/pipes2.dmi', "filter-spark"))
		sleep(6)
		src.updateicon()
...
/obj/machinery/pipefilter/proc/updateicon()
	...
	if(stat & NOPOWER)
		...
		if(emagged)	//only show if powered because presumeably its the interface that has been fried
			add_overlay(image('icons/obj/pipes/pipes2.dmi', "filter-emag"))
		if (src.f_mask & (GAS_N2O|GAS_PL))
			add_overlay(image('icons/obj/pipes/pipes2.dmi', "filter-tox"))
		if (src.f_mask & GAS_O2)
			add_overlay(image('icons/obj/pipes/pipes2.dmi', "filter-o2"))
		if (src.f_mask & GAS_N2)
			add_overlay(image('icons/obj/pipes/pipes2.dmi', "filter-n2"))
		if (src.f_mask & GAS_CO2)
			add_overlay(image('icons/obj/pipes/pipes2.dmi', "filter-co2"))
	if(!locked)
		add_overlay(image('icons/obj/pipes/pipes2.dmi', "filter-open"))
		if(bypassed)	//should only be bypassed if unlocked
			add_overlay(image('icons/obj/pipes/pipes2.dmi', "filter-bypass"))