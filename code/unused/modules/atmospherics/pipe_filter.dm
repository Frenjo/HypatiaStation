...
/obj/machinery/pipefilter/attackby(obj/item/W, mob/user as mob)
	...
	if(istype(W, /obj/item/card/emag) && (!emagged))
		...
		src.overlays += image('icons/obj/pipes/pipes2.dmi', "filter-spark")
		sleep(6)
		src.updateicon()
...
/*	if(stat & NOPOWER)
	...
	user.machine = src
	var/dat = "Filter Release Rate:<BR>\n<A href='byond://?src=\ref[src];fp=-[num2text(src.maxrate, 9)]'>M</A> <A href='byond://?src=\ref[src];fp=-100000'>-</A> <A href='byond://?src=\ref[src];fp=-10000'>-</A> <A href='byond://?src=\ref[src];fp=-1000'>-</A> <A href='byond://?src=\ref[src];fp=-100'>-</A> <A href='byond://?src=\ref[src];fp=-1'>-</A> [src.f_per] <A href='byond://?src=\ref[src];fp=1'>+</A> <A href='byond://?src=\ref[src];fp=100'>+</A> <A href='byond://?src=\ref[src];fp=1000'>+</A> <A href='byond://?src=\ref[src];fp=10000'>+</A> <A href='byond://?src=\ref[src];fp=100000'>+</A> <A href='byond://?src=\ref[src];fp=[num2text(src.maxrate, 9)]'>M</A><BR>\n"
	for (var/i = 1; i <= gases.len; i++)
		dat += "[gases[i]]: <A href='byond://?src=\ref[src];tg=[1 << (i - 1)]'>[(src.f_mask & 1 << (i - 1)) ? "Releasing" : "Passing"]</A><BR>\n"
	if(gas.total_moles())
...
/obj/machinery/pipefilter/proc/updateicon()
	...
	if(stat & NOPOWER)
		...
		if(emagged)	//only show if powered because presumeably its the interface that has been fried
			src.overlays += image('icons/obj/pipes/pipes2.dmi', "filter-emag")
		if (src.f_mask & (GAS_N2O|GAS_PL))
			src.overlays += image('icons/obj/pipes/pipes2.dmi', "filter-tox")
		if (src.f_mask & GAS_O2)
			src.overlays += image('icons/obj/pipes/pipes2.dmi', "filter-o2")
		if (src.f_mask & GAS_N2)
			src.overlays += image('icons/obj/pipes/pipes2.dmi', "filter-n2")
		if (src.f_mask & GAS_CO2)
			src.overlays += image('icons/obj/pipes/pipes2.dmi', "filter-co2")
	if(!locked)
		src.overlays += image('icons/obj/pipes/pipes2.dmi', "filter-open")
		if(bypassed)	//should only be bypassed if unlocked
			src.overlays += image('icons/obj/pipes/pipes2.dmi', "filter-bypass")