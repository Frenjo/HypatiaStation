...
// The SPACEMAN_DMM define is set by the linter and other tooling when it runs.
	...
	/*
	 * Protected procs can only be called by things of the same type *or subtypes*.
	 */
	#define PROTECTED_PROC(X) set SpacemanDMM_protected_proc = X

	/*
	 * Final vars cannot be overriden by subtypes that inherit them.
	 */
	#define VAR_FINAL var/SpacemanDMM_final

	/*
	 * Private vars can only be accessed by things of exactly the same type.
	 */
	#define VAR_PRIVATE var/SpacemanDMM_private

	/*
	 * Protected vars can only be accessed by things of the same type *or subtypes*.
	 */
