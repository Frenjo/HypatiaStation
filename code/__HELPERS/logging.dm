...
/proc/log_admin(text)
	...
	if(CONFIG_GET(/decl/configuration_entry/log_admin))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]ADMIN: [text][global.log_end]")

/proc/log_debug(text)
	if(CONFIG_GET(/decl/configuration_entry/log_debug))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]DEBUG: [text][global.log_end]")
...
/proc/log_game(text)
	if(CONFIG_GET(/decl/configuration_entry/log_game))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]GAME: [text][global.log_end]")

/proc/log_vote(text)
	if(CONFIG_GET(/decl/configuration_entry/log_vote))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]VOTE: [text][global.log_end]")

/proc/log_access(text)
	if(CONFIG_GET(/decl/configuration_entry/log_access))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]ACCESS: [text][global.log_end]")

/proc/log_say(text)
	if(CONFIG_GET(/decl/configuration_entry/log_say))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]SAY: [text][global.log_end]")

/proc/log_ooc(text)
	if(CONFIG_GET(/decl/configuration_entry/log_ooc))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]OOC: [text][global.log_end]")

/proc/log_whisper(text)
	if(CONFIG_GET(/decl/configuration_entry/log_whisper))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]WHISPER: [text][global.log_end]")

/proc/log_emote(text)
	if(CONFIG_GET(/decl/configuration_entry/log_emote))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]EMOTE: [text][global.log_end]")

/proc/log_attack(text)
	if(CONFIG_GET(/decl/configuration_entry/log_attack))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]ATTACK: [text][global.log_end]") //Seperate attack logs? Why?  FOR THE GLORY OF SATAN!

/proc/log_adminsay(text)
	if(CONFIG_GET(/decl/configuration_entry/log_adminchat))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]ADMINSAY: [text][global.log_end]")

/proc/log_adminwarn(text)
	if(CONFIG_GET(/decl/configuration_entry/log_adminwarn))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]ADMINWARN: [text][global.log_end]")

/proc/log_pda(text)
	if(CONFIG_GET(/decl/configuration_entry/log_pda))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]PDA: [text][global.log_end]")

/proc/log_to_dd(text)
	TO_WORLD_LOG(text) //this comes before the config check because it can't possibly runtime
	if(CONFIG_GET(/decl/configuration_entry/log_world_output))
		TO_FILE(GLOBL.diary, "\[[time_stamp()]]DD_OUTPUT: [text][global.log_end]")

/proc/log_misc(text)
	TO_FILE(GLOBL.diary, "\[[time_stamp()]]MISC: [text][global.log_end]")