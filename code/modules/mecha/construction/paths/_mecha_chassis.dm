// Chassis
/datum/component/construction/mecha_chassis/custom_action(step, obj/item/used_item, mob/living/user)
	var/atom/holder = parent_datum
	holder.balloon_alert_visible("connected [used_item.name]")
	holder.add_overlay(image(used_item.icon, used_item.icon_state + "+o"))
	user.drop_item()
	qdel(used_item)
	return TRUE

/datum/component/construction/mecha_chassis/action(obj/item/used_item, mob/living/user)
	return check_all_steps(used_item, user)

/datum/component/construction/mecha_chassis/spawn_result()
	var/atom/holder = parent_datum
	var/obj/item/mecha_part/chassis/chassis = holder
	holder.icon = chassis.target_icon
	holder.overlays.len = 0
	chassis.RemoveComponent(src)
	chassis.AddComponent(result)
	qdel(src)