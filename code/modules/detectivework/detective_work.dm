...
/obj/machinery/computer/forensic_scanning/initialise()
	. = ..()
			...
			if(scanning)
				if(scan_process)
					...
					if(isai)
						...
						dat += "Scan Object: <a href='byond://?src=\ref[src];operation=eject'>{[scanning.name]}</a><br>"
				...
				if(isai)
					...
					dat += "<a href='byond://?src=\ref[src];operation=insert'>{No Object Inserted}</a><br>"
				dat += "{Scan} <a href='byond://?src=\ref[src];operation=print'>{Print}</a><br>"
			dat += {"<a href='byond://?src=\ref[src];operation=database'>{Access Database}</a><br><br>"
			<tt>[scan_data]</tt>"}
