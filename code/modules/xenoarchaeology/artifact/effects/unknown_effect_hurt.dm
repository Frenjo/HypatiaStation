...
/datum/artifact_effect/hurt/DoEffectTouch(mob/toucher)
	if(toucher)
		...
		if(iscarbon(toucher) && prob(weakness * 100))
			...
			to_chat(C, SPAN_WARNING("A painful discharge of energy strikes you!"))
	...
	if(holder)
		for(var/mob/living/carbon/C in range(src.effectrange, holder))
			...
			if(prob(weakness * 100))
				if(prob(10))
					to_chat(C, SPAN_WARNING("You feel a painful force radiating from something nearby."))
...
/datum/artifact_effect/hurt/DoEffectPulse()
	if(holder)
		for(var/mob/living/carbon/C in range(effectrange, holder))
			var/weakness = GetAnomalySusceptibility(C)
			if(prob(weakness * 100))
				to_chat(C, SPAN_WARNING("A wave of painful energy strikes you!"))
