	...
	Announce()
		if(!virus)
			for(var/mob/living/carbon/human/H in GLOBL.mob_list)
				if(length(H.virus2) || H.stat == DEAD || prob(30))
					continue
				if(prob(100))	// no lethal diseases outside virus mode!
					...
					if(prob(20))//don't want people to know that the virus alert = greater virus
						priority_announce(
							"Probable outbreak of level [rand(1,6)] viral biohazard aboard [station_name()]. All personnel must contain the outbreak.",
							"Virus Alert"
						)
				else
					...
					if(prob(80))
						priority_announce(
							"Probable outbreak of level [rand(2,9)] viral biohazard aboard [station_name()]. All personnel must contain the outbreak.",
							"Virus Alert"
						)
				break
