/datum/round_event/pda_spam
	...
	var/static/list/bet_messages = list(
		"Triple deposits are waiting for you at MaxBet Online when you register to play with us.",
	...
	var/static/list/dating_messages = list(
		"Your profile caught my attention and I wanted to write and say hello (QuickDating).",
	...
	var/static/list/payment_messages = list(
		"Luxury watches for Blowout sale prices!",
	...
	var/static/list/dysfunctional_messages = list(
		"DR MAXMAN: REAL Doctors, REAL Science, REAL Results!",
	...
	var/static/list/beneficiary_messages = list(
	...
	var/static/list/morale_messages = list(
		"The NanoTrasen Morale Division wishes to provide you with quality entertainment sites.",
	...
	var/static/list/prize_messages = list(
		"You have won tickets to the newest ACTION JAXSON MOVIE!",
		...
		if(prob(2))
			...
			switch(pick(1, 2, 3, 4, 5, 6, 7))
				if(1)
					...
					message = pick(bet_messages)
				if(2)
					sender = pick(\
						300;"QuickDatingSystem", 200;"Find your russian bride",\
						50;"Tajaran beauties are waiting", 50;"Find your secret skrell crush", 50;"Beautiful soghun brides"\
					)
					message = pick(dating_messages)
				if(3)
					...
					message = pick(payment_messages)
				if(4)
					sender = pick("Buy Dr. Maxman","Having dysfuctional troubles?")
					message = pick(dysfunctional_messages)
				if(5)
			...
			for(var/mob/O in hearers(3, P.loc))
				if(!P.silent)
					O.show_message(text("[html_icon(P)] *[P.ttone]*"))
			...
			if(isnotnull(L))
				to_chat(L, "[html_icon(P)] <b>Message from [sender] (Unknown / spam?), </b>\"[message]\" (Unable to Reply)")
