					...
					if(istype(my_atom, /obj/item/slime_core))
						...
						if(ME.Uses <= 0) // give the notification that the slime core is dead
							for(var/mob/M in viewers(4, GET_TURF(my_atom)))
								M << "\blue [icon2html(my_atom, M)] The innards begin to boil!"
				*/
					...
					if(istype(my_atom, /obj/item/slime_extract))
						...
						if(ME2.uses <= 0) // give the notification that the slime core is dead
							my_atom.visible_message(SPAN_INFO("[icon2html(my_atom, viewers(my_atom))] The [my_atom]'s power is consumed in the reaction."))
