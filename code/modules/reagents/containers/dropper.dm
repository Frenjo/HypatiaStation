	...
	if(filled)
		...
		if(ismob(target))
			...
			for(var/mob/O in viewers(world.view, user))
				O.show_message(SPAN_DANGER("[user] is trying to squirt something into [target]'s eyes!"), 1)
			...
			if(ishuman(target))
				...
				if(safe_thing)
					...
					for(var/mob/O in viewers(world.view, user))
						O.show_message(SPAN_DANGER("[user] tries to squirt something into [target]'s eyes, but fails!"), 1)
					spawn(5)
			...
			for(var/mob/O in viewers(world.view, user))
				O.show_message(SPAN_DANGER("[user] squirts something into [target]'s eyes!"), 1)
