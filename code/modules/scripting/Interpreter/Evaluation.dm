...
/n_Interpreter/proc/Eval(node/expression/exp)
	...
	else if(istype(exp, /node/expression/_operator))
...
/n_Interpreter/proc/EvalOperator(node/expression/_operator/exp)
	if(istype(exp, /node/expression/_operator/binary))
		var/node/expression/_operator/binary/bin=exp
		switch(bin.type)
			if(/node/expression/_operator/binary/Equal)
				return Equal(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/NotEqual)
				return NotEqual(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/Greater)
				return Greater(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/Less)
				return Less(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/GreaterOrEqual)
				return GreaterOrEqual(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/LessOrEqual)
				return LessOrEqual(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/LogicalAnd)
				return LogicalAnd(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/LogicalOr)
				return LogicalOr(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/LogicalXor)
				return LogicalXor(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/BitwiseAnd)
				return BitwiseAnd(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/BitwiseOr)
				return BitwiseOr(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/BitwiseXor)
				return BitwiseXor(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/Add)
				return Add(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/Subtract)
				return Subtract(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/Multiply)
				return Multiply(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/Divide)
				return Divide(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/Power)
				return Power(Eval(bin.exp), Eval(bin.exp2))
			if(/node/expression/_operator/binary/Modulo)
				return Modulo(Eval(bin.exp), Eval(bin.exp2))
			else
				RaiseError(new/runtimeError/UnknownInstruction())
	else
		switch(exp.type)
			if(/node/expression/_operator/unary/Minus)
				return Minus(Eval(exp.exp))
			if(/node/expression/_operator/unary/LogicalNot)
				return LogicalNot(Eval(exp.exp))
			if(/node/expression/_operator/unary/BitwiseNot)
				return BitwiseNot(Eval(exp.exp))
			if(/node/expression/_operator/unary/group)
				return Eval(exp.exp)
			else
				RaiseError(new/runtimeError/UnknownInstruction())
