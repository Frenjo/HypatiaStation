...
/datum/preferences/proc/update_preview_icon()		//seriously. This is horrendous.
	...
	if(current_species && current_species.name == "Plasmalin")
		clothes_s = new /icon('icons/mob/uniform.dmi', "plasmalin_s")
		clothes_s.Blend(new /icon('icons/mob/head.dmi', "rig0-standard-plasmalin"), ICON_OVERLAY)
	...
	else if(job_civilian_low & ASSISTANT)//This gives the preview icon clothes depending on which job(if any) is set to 'high'
			...
			if(CARGOTECH)
				...
				switch(backbag)
					...
					if(4)
						clothes_s.Blend(new /icon('icons/mob/back.dmi', "satchel"), ICON_OVERLAY)
			if(MININGFOREMAN)
				clothes_s = new /icon('icons/mob/uniform.dmi', "overalls_s")
				clothes_s.Blend(new /icon('icons/mob/feet.dmi', "black"), ICON_UNDERLAY)
				clothes_s.Blend(new /icon('icons/mob/hands.dmi', "bgloves"), ICON_UNDERLAY)
				switch(backbag)
					if(2)
