...
#define HUMAN_CRIT_MAX_OXYLOSS ( (tickerProcess.getLastTickerTimeDuration()) / 5)
...
*/
	...
	proc/handle_regular_hud_updates()
		...
		client.screen.Remove(global_hud.blurry, global_hud.druggy, global_hud.vimpaired, global_hud.darkMask, global_hud.nvg, global_hud.scig)
		...
		if( stat == DEAD )
			...
			if(glasses)
				...
				// If we're wearing purple goggles, give a purple overlay. -Frenjo
				if(istype(G, /obj/item/clothing/glasses/science))
					client.screen += global_hud.scig
...
//				else									rest.icon_state = "rest0"
			...
			if(blind)
				if(blinded)		blind.invisibility = 0 // Changed blind.layer to blind.invisibility to become compatible with not-2014 BYOND. -Frenjo
				else			blind.invisibility = 101 // Changed blind.layer to blind.invisibility to become compatible with not-2014 BYOND. -Frenjo
