...
#define HUMAN_CRIT_MAX_OXYLOSS ( (global.ticker_process.getLastTickerTimeDuration()) / 5)
...
/mob/living/carbon/human/Life()
	...
	*/
	...
	if(stat != DEAD && !in_stasis)
		if(global.air_master.current_cycle % 4 == 2 || failed_last_breath) 	//First, resolve location and get a breath
		...
		for(var/mob/living/M in stomach_contents)
			...
			if(iscarbon(M) && stat != DEAD)
				if(M.stat == DEAD)
					...
					qdel(M)
					continue
				if(global.air_master.current_cycle % 3 == 1)
...
/mob/living/carbon/human/proc/handle_pulse()
	...
	for(var/datum/reagent/R in reagents.reagent_list)
		if(R.id in global.bradycardics)
			if(temp <= PULSE_THREADY && temp >= PULSE_NORM)
				temp--

		if(R.id in global.tachycardics)
			if(temp <= PULSE_FAST && temp >= PULSE_NONE)
				temp++

		if(R.id in global.heartstopper) //To avoid using fakedeath
			temp = PULSE_NONE

		if(R.id in global.cheartstopper) //Conditional heart-stoppage
