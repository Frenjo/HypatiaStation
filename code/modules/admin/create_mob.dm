...
/datum/admins/proc/create_mob(mob/user)
	if(isnull(GLOBL.create_mob_html))
		GLOBL.create_mob_html = file2text('html/misc/create_object.html')
		GLOBL.create_mob_html = replacetext(GLOBL.create_mob_html, "null /* object types */", "\"[jointext(typesof(/mob), ";")]\"")

	SHOW_BROWSER(user, replacetext(GLOBL.create_mob_html, "/* ref src */", "\ref[src]"), "window=create_mob;size=425x475")