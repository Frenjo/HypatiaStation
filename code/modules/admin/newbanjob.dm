...
/proc/_jobban_isbanned(client/clientvar, rank)
	...
	if(guest_jobbans(rank))
		if(CONFIG_GET(/decl/configuration_entry/guest_jobban) && IsGuestKey(key))
			return 1
	Banlistjob.cd = "/base"
...
/proc/AddBanjob(ckey, computerid, reason, bannedby, temp, minutes, rank)
	...
	if(rank == "Heads")
		...
		AddBanjob(ckey, computerid, reason, bannedby, temp, minutes, "Quartermaster")
		return 1
	if(rank == "Security")
		AddBanjob(ckey, computerid, reason, bannedby, temp, minutes, "Head of Security")
		...
		AddBanjob(ckey, computerid, reason, bannedby, temp, minutes, "Security Paramedic")
	...
	if(rank == "Cargo")
		AddBanjob(ckey, computerid, reason, bannedby, temp, minutes, "Quartermaster")
		AddBanjob(ckey, computerid, reason, bannedby, temp, minutes, "Cargo Technician")
		AddBanjob(ckey, computerid, reason, bannedby, temp, minutes, "Mining Foreman")
...
/datum/admins/proc/unjobbanpanel()
	...
	for(var/A in Banlistjob.dir)
		count++
		Banlistjob.cd = "/base/[A]"
		dat += text("<tr><td><A href='byond://?src=\ref[src];unjobbanf=[Banlistjob["key"]][Banlistjob["id"]][Banlistjob["rank"]]'>(U)</A> Key: <B>[Banlistjob["key"]] </B>Rank: <B>[Banlistjob["rank"]]</B></td><td> ([Banlistjob["temp"] ? "[GetBanExpjob(Banlistjob["minutes"]) ? GetBanExpjob(Banlistjob["minutes"]) : "Removal pending" ]" : "Permaban"])</td><td>(By: [Banlistjob["bannedby"]])</td><td>(Reason: [Banlistjob["reason"]])</td></tr>")
...
/*/datum/admins/proc/permjobban(ckey, computerid, reason, bannedby, temp, minutes, rank)
	if(AddBanjob(ckey, computerid, reason, usr.ckey, 0, 0, job))
		...
		if(CONFIG_GET(/decl/configuration_entry/banappeals))
