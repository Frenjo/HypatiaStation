	...
	if(href_list["makeAntag"])
		switch(href_list["makeAntag"])
			...
			if("9")
				...
				new /datum/round_event/alien_infestation(3)
			if("10")
		...
		priority_announce(
			...
			null, null, ANNOUNCEMENT_TYPE_PRIORITY
		)
		...
		switch(href_list["simplemake"])
			...
			if("slime")			M.change_mob_type( /mob/living/simple/slime , null, null, delmob )
			if("adultslime")		M.change_mob_type( /mob/living/simple/slime/adult , null, null, delmob )
...
*/
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				message_admins("\blue[usr.client.ckey] has banned [M.ckey].<br>Reason: [reason]<br>This will be removed in [mins] minutes.")
			...
			if("No")
				...
				message_admins("\blue[usr.client.ckey] has banned [M.ckey].<br>Reason: [reason]<br>This is a permanent ban.")
		...
		spawn(50)
			to_chat(M, SPAN_INFO("You have been sent to the Thunderdome."))
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
		...
		//Job + antagonist
		if(M.mind)
			special_role_description = "Role: <b>[M.mind.assigned_role]</b>; Antagonist: <font color='red'><b>[jointext(M.mind.special_roles, ", ")]</b></font>; Has been rev: [(M.mind.has_been_rev)?"Yes":"No"]"
		else
...
/*				if("shockwave")
			...
			if("wave")
				...
				new /datum/round_event/meteor_wave()
				message_admins("[key_name_admin(usr)] has spawned a meteor wave.", 1)

			if("gravanomalies")
				feedback_inc("admin_secrets_fun_used", 1)
				feedback_add_details("admin_secrets_fun_used", "GA")
				new /datum/round_event/gravitational_anomaly()
				message_admins("[key_name_admin(usr)] has spawned a gravitational anomaly.", 1)
			...
			if("timeanomalies") //dear god this code was awful :P Still needs further optimisation
				...
				message_admins("[key_name_admin(usr)] has spawned wormholes.", 1)

			if("goblob")
				feedback_inc("admin_secrets_fun_used", 1)
				feedback_add_details("admin_secrets_fun_used", "BL")
				mini_blob_event()
				message_admins("[key_name_admin(usr)] has spawned a blob.", 1)
			...
			if("comms_blackout")
				...
				if(answer == "Yes")
					...
					new /datum/round_event/communications_blackout(TRUE)
				message_admins("[key_name_admin(usr)] triggered a communications blackout.", 1)
			if("processor_overload")
				feedback_inc("admin_secrets_fun_used", 1)
				feedback_add_details("admin_secrets_fun_used", "PO")
				new /datum/round_event/processor_overload()
				message_admins("[key_name_admin(usr)] triggered a processor overload.", 1)
			if("spaceninja")
				feedback_inc("admin_secrets_fun_used", 1)
				feedback_add_details("admin_secrets_fun_used", "SN")
				if(!GLOBL.toggle_space_ninja)
					return
				new /datum/round_event/space_ninja()
				message_admins("[key_name_admin(usr)] attempted to send in a space ninja.", 1)
			if("carp")
				feedback_inc("admin_secrets_fun_used",1)
			...
			if("immovable_rod")
				...
				new /datum/round_event/immovable_rod()
			...
			if("electrical_storm_large")
				...
				new /datum/round_event/storm/electrical/large()
				message_admins("[key_name_admin(usr)] triggered a severe electrical storm.", 1)
			if("space_dust")
				feedback_inc("admin_secrets_fun_used", 1)
				feedback_add_details("admin_secrets_fun_used", "SD")
				var/severity = href_list["severity"]
				switch(severity)
					if("weak")
						new /datum/round_event/space_dust/weak()
					if("normal")
						new /datum/round_event/space_dust()
					if("strong")
						new /datum/round_event/space_dust/strong()
					if("super")
						new /datum/round_event/space_dust/super()
				message_admins("[key_name_admin(usr)] triggered [severity] space dust.", 1)
			if("portal_storm")
				feedback_inc("admin_secrets_fun_used", 1)
				feedback_add_details("admin_secrets_fun_used", "SD")
				...
				switch(faction)
					if("syndie")
						new /datum/round_event/storm/portal/syndicate()
					if("cult")
						new /datum/round_event/storm/portal/narsie()
				message_admins("[key_name_admin(usr)] triggered a [faction] portal storm.", 1)
		if(usr)
