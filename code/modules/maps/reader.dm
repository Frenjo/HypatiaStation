...
/dmm_suite/proc/load_map(dmm_file as file, z_offset as num, y_offset as num, x_offset as num)
	...
	TO_FILE(debug_file, "Starting Map Load @ ([x_offset], [y_offset], [z_offset]).")
	...
	TO_FILE(debug_file, "	Building turf array.")
		...
		for(var/grid_position = 1; grid_position != 0; grid_position = findtext(z_level, "\n", grid_position) + 1)
			...
			if(!y_depth)
				...
				if(y_depth != round(y_depth, 1))
					TO_FILE(debug_file, "	Warning: y_depth is not a round number")
...
/dmm_suite/proc/parse_grid(model as text, x_coordinate as num, y_coordinate as num, z_coordinate as num)
	...
	if(debug_output)
		TO_FILE(debug_file, "	Now debugging turf: [model] ([x_coordinate], [y_coordinate], [z_coordinate])")
	...
	for(var/data_position = 1, next_position || data_position != 1, data_position = next_position + 1)
		...
		if(debug_output)
			TO_FILE(debug_file, "		Current Line: [full_def] -- ([data_position] - [next_position])")
		...
		if(attribute_position)
			...
			if(debug_output)
				TO_FILE(debug_file, "		Atom Def: [atom_def]")
				TO_FILE(debug_file, "		Parameters: [full_def]")
		...
		if(debug_output)
			...
			TO_FILE(debug_file, return_data)
...
/dmm_suite/proc/trim_text(what as text)
...
/dmm_suite/proc/get_list(text, list/text_strings)
