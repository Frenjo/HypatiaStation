...
/obj/machinery/suspension_gen/proc/activate()
	...
	switch(field_type)
		if("carbon")
			...
			for(var/mob/living/carbon/C in T)
				C.weakened += 5
				C.visible_message(
					SPAN_INFO("[icon2html(C, viewers(C))] [C] begins to float in the air!"),
		...
		if("iron")
			...
			for(var/mob/living/silicon/R in T)
				R.weakened += 5
				R.visible_message(
					SPAN_INFO("[icon2html(R, viewers(R))] [R] begins to float in the air!"),
					"You feel tingly and light, but it is difficult to move."
				)
			//
	...
	visible_message(SPAN_INFO("[icon2html(src, viewers(src))] [src] activates with a low hum."))
	...
	if(collected)
		suspension_field.icon_state = "energynet"
		suspension_field.add_overlay("shield2")
		visible_message(SPAN_INFO("[icon2html(suspension_field, viewers(src))] [suspension_field] gently absconds [collected > 1 ? "something" : "several things"]."))
	else
...
/obj/machinery/suspension_gen/proc/deactivate()
	...
	visible_message(SPAN_INFO("[icon2html(src, viewers(src))] [src] deactivates with a gentle shudder."))
