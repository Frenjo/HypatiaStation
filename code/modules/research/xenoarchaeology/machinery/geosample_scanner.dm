	...
	if(scanning)
		if(!scanned_item || scanned_item.loc != src)
			...
			if(scanner_seal_integrity <= 0 || (scanner_temperature >= 1273 && !rad_shield))
				stop_scanning()
				src.visible_message(SPAN_INFO("[html_icon(src)] buzzes unhappily. It has failed mid-scan!"), 2)

			if(prob(5))
				src.visible_message(SPAN_INFO("[html_icon(src)] [pick("whirrs", "chuffs", "clicks")][pick(" excitedly", " energetically", " busily")]."), 2)
	else
		//gradually cool down over time
		if(scanner_temperature > 0)
			scanner_temperature = max(scanner_temperature - 5 - 10 * rand(), 0)
		if(prob(0.75))
			src.visible_message(SPAN_INFO("[html_icon(src)] [pick("plinks", "hisses")][pick(" quietly", " softly", " sadly", " plaintively")]."), 2)
...
/obj/machinery/radiocarbon_spectrometer/proc/complete_scan()
	src.visible_message(SPAN_INFO("[html_icon(src)] makes an insistent chime."), 2)
