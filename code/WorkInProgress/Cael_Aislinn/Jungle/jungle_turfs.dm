...
/turf/simulated/jungle/rock/New()
	spawn(1)
		var/turf/T
		if(!istype(get_step(src, NORTH), /turf/simulated/jungle/rock) && !istype(get_step(src, NORTH), /turf/simulated/wall/reinforced/riveted))
			T = get_step(src, NORTH)
			if(T)
				T.overlays += image(icon, "rock_side_s")
		if(!istype(get_step(src, SOUTH), /turf/simulated/jungle/rock) && !istype(get_step(src, SOUTH), /turf/simulated/wall/reinforced/riveted))
			T = get_step(src, SOUTH)
			if(T)
				T.overlays += image(icon, "rock_side_n", layer = 6)
		if(!istype(get_step(src, EAST), /turf/simulated/jungle/rock) && !istype(get_step(src, EAST), /turf/simulated/wall/reinforced/riveted))
			T = get_step(src, EAST)
			if(T)
				T.overlays += image(icon, "rock_side_w", layer = 6)
		if(!istype(get_step(src, WEST), /turf/simulated/jungle/rock) && !istype(get_step(src, WEST), /turf/simulated/wall/reinforced/riveted))
			T = get_step(src, WEST)
			if(T)
