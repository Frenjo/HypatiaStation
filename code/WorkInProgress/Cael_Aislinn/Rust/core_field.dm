...
/obj/effect/rust_em_field/process()
	...
	if(held_plasma.gas[GAS_PLASMA] < transfer_ratio * 1000)
		...
		//
		plasma_captured.gas[GAS_PLASMA] = round(gas_covered.gas[GAS_PLASMA] * transfer_ratio)
		...
		//
		gas_covered.adjust_gas(GAS_PLASMA, -plasma_captured.gas[GAS_PLASMA])
		//
	...
	//SPECIFIC_HEAT_TOXIN
	if(mega_energy > 0 && held_plasma.gas[GAS_PLASMA])
	...
	/*if( held_plasma.toxins > (MOLES_CELLSTANDARD * 7) * (50 / field_strength) )
		LosePlasma()*/
	if(held_plasma.gas[GAS_PLASMA] > 1)
		...
		//
		plasma_lost.gas[GAS_PLASMA] = held_plasma.gas[GAS_PLASMA] * loss_ratio
		//plasma_lost.update_values()
		held_plasma.gas[GAS_PLASMA] -= held_plasma.gas[GAS_PLASMA] * loss_ratio
		...
		held_plasma.gas[GAS_PLASMA] = null
		//held_plasma.update_values()
