...
/obj/effect/rust_em_field/process()
	...
	if(held_plasma.gas["plasma"] < transfer_ratio * 1000)
		...
		//
		plasma_captured.gas["plasma"] = round(gas_covered.gas["plasma"] * transfer_ratio)
		...
		//
		gas_covered.adjust_gas("plasma", -plasma_captured.gas["plasma"])
		//
	...
	//SPECIFIC_HEAT_TOXIN
	if(mega_energy > 0 && held_plasma.gas["plasma"])
	...
	if(held_plasma.gas["plasma"] > 1)
		...
		//
		plasma_lost.gas["plasma"] = held_plasma.gas["plasma"] * loss_ratio
		//plasma_lost.update_values()
		held_plasma.gas["plasma"] -= held_plasma.gas["plasma"] * loss_ratio
