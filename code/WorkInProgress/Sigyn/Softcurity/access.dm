...
/var/const/access_tcomsat = 61 // has access to the entire telecoms satellite / machinery
...
/obj/proc/allowed(mob/M)
	...
	if(issilicon(M))
		...
		if(src.check_access(H.get_active_hand()) || src.check_access(H.id_store))
			return 1
	else if(ismonkey(M) || istype(M, /mob/living/carbon/alien/humanoid))
