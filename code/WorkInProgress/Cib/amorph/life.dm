...
/mob/living/carbon/amorph
	proc
		...
		update_canmove()
			if(paralysis || stunned || weakened || buckled || (changeling && changeling.changeling_fakedeath)) canmove = FALSE
			else canmove = TRUE
		...
		handle_regular_status_updates()
			...
			if(health < CONFIG_GET(/decl/configuration_entry/health_threshold_dead) && stat != 2)
				death()
		...
		handle_regular_hud_updates()
			...
			if (src.stat == 2 || (MUTATION_XRAY in mutations))
				src.sight |= SEE_TURFS
				src.sight |= SEE_MOBS
				src.sight |= SEE_OBJS
				src.see_in_dark = 8
