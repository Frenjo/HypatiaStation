...
// Deprecated! See _global_vars/database.dm for new configuration vars
/*
...
/DBConnection/proc/SelectDB(database_name, dbi)
	...
	return Connect("[dbi ? "[dbi]" : "dbi:mysql:[database_name]:[CONFIG_GET(/decl/configuration_entry/sql_address)]:[CONFIG_GET(/decl/configuration_entry/sql_port)]"]", user, password)
