...
/vs_control/proc/ChangeSetting(mob/user, ch)
	...
	to_chat(world, SPAN_NOTICE("[key_name(user)] changed the setting [display_description] to [newvar]."))
	if(ch in plc.settings)
...
/vs_control/proc/SetDefault(mob/user)
	...
	to_chat(world, SPAN_NOTICE("[key_name(user)] changed the global plasma/ZAS settings to \"[def]\""))
