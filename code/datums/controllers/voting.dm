...
/datum/controller/vote/proc/announce_result()
	...
	if(winners.len > 0)
		if(winners.len > 1)
			if(mode != "gamemode" || global.ticker.hide_mode == 0) // Here we are making sure we don't announce potential game modes
		...
		if((mode == "gamemode" && . == "extended") || global.ticker.hide_mode == 0) // Announce Extended gamemode, but not other gamemodes
